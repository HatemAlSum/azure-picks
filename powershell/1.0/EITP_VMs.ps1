...
. "D:\c9\powershell\1.0\Configure-AzureWinRMHTTPS.ps1"

$startTime=Get-Date
###### Global Variables
$ResourceGroupName = "EITP"
$AvalabiltySetName = "EITPIIS"
$Location = "westeurope"
###### Storage	##storageName must be small letters!!
$StorageName = "eitpstorageacc"
$StorageType = "Standard_LRS"
###### Network
$Subnet1Name = "EITPSubNet1"
$VNetNamepub = "EITPVNet"
...
###### Compute
$VMList = ('EITP-IIS-Node1','EITP-IIS-Node2')
...
$nic1 = Get-AzureRmNetworkInterface -resourcegroupname $ResourceGroupName| where { $_.Name -like 'EITP-IIS-Node1*' }
$nic2 = Get-AzureRmNetworkInterface -resourcegroupname $ResourceGroupName| where { $_.Name -like 'EITP-IIS-Node2*' }
...
{
   Get-AzureRmRemoteDesktopFile -ResourceGroupName $ResourceGroupName -Name $VMName -LocalPath "D:\$vmname.rdp"
}
#Install IIS on Both Servers
Invoke-Command -ComputerName $DNSName -Port 52122 -Credential $credential -UseSSL -SessionOption (New-PsSessionOption -SkipCACheck -SkipCNCheck)  -FilePath D:\c9\powershell\1.0\Install_IIS.ps1
Invoke-Command -ComputerName $DNSName -Port 52222 -Credential $credential -UseSSL -SessionOption (New-PsSessionOption -SkipCACheck -SkipCNCheck)  -FilePath D:\c9\powershell\1.0\Install_IIS.ps1 

#########################################
#$SQLAVS ="MOFSQLAS"
$StorageName = "eitpsqlsacc"
$StorageType = "Standard_LRS"
$InterfaceName = "EITP-SQL2012Nic"
###### Compute
$VMName = "EITP-SQL-Node"
$ComputerName = "EITP-SQL-Node"
...
$nic3 = Get-AzureRmNetworkInterface -resourcegroupname $ResourceGroupName| where { $_.Name -like 'EITP-SQL*' }
...
Set-AzureRmVMSqlServerExtension -ResourceGroupName $ResourceGroupName -VMName $VMName
Update-AzureRmVM -ResourceGroupName $ResourceGroupName -VMName $VMName
